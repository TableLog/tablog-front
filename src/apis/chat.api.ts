...
export const getMyChatRooms = async () => {
  return await instance.get(`${CHAT_URL}/rooms`);
};

// 채팅방 메시지를 정렬 옵션과 함께 조회합니다
export const getChatRoomMessages = async (roomId: string, sortOption: string) => {
  return await instance.get(`${CHAT_URL}/rooms/${roomId}/messages?sortOption=${sortOption}`);
};
